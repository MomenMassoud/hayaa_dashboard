...
class BubbleBody extends StatefulWidget{
  _BubbleBody createState()=>_BubbleBody();
}


class _BubbleBody extends State<BubbleBody>{
  final FirebaseFirestore _firestore=FirebaseFirestore.instance;
  @override
  Widget build(BuildContext context) {
    final double screenWidth = MediaQuery.of(context).size.width;
    return StreamBuilder<QuerySnapshot>(
      stream: _firestore.collection('store').where('cat',isEqualTo: 'bubble').snapshots(),
      builder: (context,snapshot){
        ...
        return Scaffold(
          body: Padding(
            ...
            child: Column(
              children: [
                Text(" Numbers of Bubble : ${store.length.toString()}"),
                const SizedBox(
                  height: 30,
                ),
                Row(
