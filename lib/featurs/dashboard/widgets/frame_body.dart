...
class FrameBody extends StatefulWidget{
  _FrameBody createState()=>_FrameBody();
}


class _FrameBody extends State<FrameBody>{
  final FirebaseFirestore _firestore=FirebaseFirestore.instance;
  @override
  Widget build(BuildContext context) {
    final double screenWidth = MediaQuery.of(context).size.width;
    return StreamBuilder<QuerySnapshot>(
      stream: _firestore.collection('store').where('cat',isEqualTo: 'frame').snapshots(),
      builder: (context,snapshot){
        ...
        return Scaffold(
          body: Padding(
            ...
            child: Column(
              children: [
                Text(" Numbers of Frame : ${store.length.toString()}"),
                const SizedBox(
                  height: 30,
                ),
                Row(
